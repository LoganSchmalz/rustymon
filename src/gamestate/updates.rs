...
use crate::engine_structures::{
    components::{HumanWalkAnimation, MovingEntity, MovingState, Player, Position, Collision},
        ...
        for (_, (pos, moving, animation)) in moving_query.iter() {
            ...
            match (moving.moving, moving.try_moving) {
                ...
                (Idle, Moving(rotation)) => {
                    if rotation != moving.rotation && moving.rotation_timer >= ROTATION_TIME {
                        ...
                        animation.play_animation(ROTATION_TIME, rotation)
                    }
                }
            ...
            match (moving.moving, moving.try_moving) {
                ...
                (Idle, new_state) | (CenterTile, new_state) => {
                    ...
                    if !animation.is_playing() {
                        let time = match moving.sprinting {
                            true => RUNNING_TIME_PER_TILE,
                            false => WALKING_TIME_PER_TILE,
                        };
                        animation.play_animation(time, moving.rotation);
